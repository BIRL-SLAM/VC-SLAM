...
class Cloud_Slam:
    ...
    def slam_run(self, goal):
        ...
        with open('/data/xjluo/cloud_udf/src/edgecloud/scripts/result/fr2_p_360_cloud_trajectory-mlp.txt', 'w') as file:
        ...
        o3d.io.write_point_cloud("/data/xjluo/cloud_udf/src/edgecloud/scripts/pcd/fr2_p_360-mlp.ply", pcd)
        #cap-udf
        ...
        if (self.new_udf):
            ...
            put_pth("test.pth", "/home/birl/Udf-Edge/Cloud-Edge-SLAM/Cloud-Edge-SLAM-master/trans_pth") #cap-udf
            ...
            result.map.edge_front_map_mnid = goal.sequence.edge_front_map_mnid
            result.map.edge_back_map_mnid  = goal.sequence.edge_back_map_mnid
        #bag = rosbag.Bag('/home/luo/cloud_udf/trans_work/cloud.bag','w')
    ...
    def train(self):
        ...
        losses = []  # 用于保存每步的损失值
        # 打开文件以写入数据
        with open('/data/xjluo/cloud_udf/src/edgecloud/scripts/result/SNR-training_log-mlp.txt', 'w') as f:
            ...
            plt.ylabel('SNR')
            plt.title('rgbd_dataset_freiburg1_room-Loss CD gradient of SNR')
            plt.legend()
            #plt.show()
            plt.savefig('/data/xjluo/cloud_udf/src/edgecloud/scripts/figure/SNR-fr1-room-mlp.png')
            ####################################################################################
            # 绘制损失图
            plt.figure(figsize=(12, 8))
            ...
            plt.legend()
            #plt.show()
            plt.savefig('/data/xjluo/cloud_udf/src/edgecloud/scripts/figure/Loss/Loss-fr1-room-mlp.png')
