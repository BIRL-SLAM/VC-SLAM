...
def new_points_des(xyz_duv, index_num, dirname, image_list, orb, per_vis):
    ...
    if isinstance(dirname, str):
        image_path = dirname + str(image_list[index_num])
        image = cv2.imread(image_path)
    else:
        image = image_list[index_num]
    ...
    # if vis:
    #    im_with_keypoints = cv2.drawKeypoints(image, kpnew, np.array([]), (255, 0, 0),
...
def downsample_mp(per_group, keyframe_index, new_point_xyz_uv, dir_name, images_list, new_visibility):
    #print(per_group)
    merge_list = []
    index_list = []
    des_index_list = []
    ...
    keyPoints1_new, des1_new, img1, querylist, query_pt_list = new_points_des(new_point_xyz_uv, index_a, dir_name, images_list, orb_detector, length)
    ...
    for index_train in range(1,len(length)):
        ...
        if index_b < index_a:
            continue
        keyPoints2_new, des2_new, img2, trainlist, train_pt_list = new_points_des(new_point_xyz_uv, index_b, dir_name, images_list, orb_detector, length)
        # time_1 += time.time() - time_1_s # <<<<<
        ...
        index_unique = np.asarray([np.argwhere(desc_list_uv == iads)[0] for iads in np.unique(desc_list_uv)])
    ...
    index_unique_all = [np.argwhere(index_list == iads)[0] for iads in np.unique(index_list)]
    ...
    for mer in merge_list:
        ...
        curr_mer_des = des_index_list_unique[ [ np.where(index_list_unique==mer_i)[0][0] for mer_i in mer ] ]  # modified
...
def downsample_feat_match(visibililty_graph, keyframe_index, new_point_xyz_uv, image_path, images_list, All_Images_XYZ_uv):
    count = 0
    # vis = False
    ...
    for per_group in range(len(new_visibility)>>2):
        ...
        none_list_per = pool.apply_async(downsample_mp, (per_group, keyframe_index, new_point_xyz_uv, image_path, images_list,new_visibility,))
    ...
    return All_Images_XYZ_uv_new

def downsample_feat_match_online(visibililty_graph, keyframe_index, new_point_xyz_uv, image_path, images_list, All_Images_XYZ_uv):
    count = 0
    # vis = False
    pool = mp.Pool(processes=20)
    time_start = time.time()
