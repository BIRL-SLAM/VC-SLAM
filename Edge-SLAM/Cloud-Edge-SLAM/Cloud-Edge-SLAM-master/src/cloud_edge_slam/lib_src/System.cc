...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    //Check settings file
    cv::FileStorage fsSettings(strSettingsFile.c_str(), cv::FileStorage::READ); //读取yaml文件
    ...
    if (!node.empty() && node.isString() && node.string() == "1.0") {
        settings_ = new Settings(strSettingsFile, mSensor); //跳转至Settings.cc,读取一系列参数

        mStrLoadAtlasFromFile = settings_->atlasLoadFile(); //没有数据
        mStrSaveAtlasToFile = settings_->atlasSaveFile(); //没有数据

        cout << (*settings_) << endl; //打印设置参数
    } else {
        settings_ = nullptr;
    ...
    node = fsSettings["loopClosing"]; //空的
    bool activeLC = true;
    if (!node.empty()) {
        activeLC = static_cast<int>(fsSettings["loopClosing"]) != 0;
    }

    mStrVocabularyFilePath = strVocFile; //赋值字典路径
    ...
    if (mStrLoadAtlasFromFile.empty()) {
        ...
        //bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile); //txt加载 判断字典文件是否正常，并读取
        ...
        mpAtlas = new Atlas(0); //跳至Atlas.cc,不清楚作用
    } else {
        //Load ORB Vocabulary
        cout << endl
             << "Loading ORB Vocabulary. This could take a while..." << endl;
    ...
    mpFrameDrawer = new FrameDrawer(mpAtlas); //意义不明
    mpMapDrawer = new MapDrawer(mpAtlas, strSettingsFile, settings_); //地图绘制
    ...
    mpCloudImageSampler = new CloudImageSampler(this, nSamplerEdgeFrontKFNum, nSamplerEdgeBackKFNum, nSamplerEdgeFrontMinTime, nSamplerEdgeBackMinTime, samplerPDKp, samplerPDKd, samplerPDth);//跳至cloudimagesampler.cc
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point> &vImuMeas, string filename) {
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed, timestamp, filename);//转至Tracking.cc的1364行
