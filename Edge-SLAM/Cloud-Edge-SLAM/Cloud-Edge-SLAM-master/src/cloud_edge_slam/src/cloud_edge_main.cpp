...
int main(int argc, char **argv) {
    ...
    getRosParam(nh, "cloud_topic_name", cloudTopicName);//获取launch中参数的值 /cloud_slam
    ...
    if (bCloudOnline) {
        if (dataType == "txt") {
            ...
            igb.RunBag(dataPath); //运行bag数据集，跳至610行
        } else {
...
void Grabber::RunBag(const string &bag_path) {
    ...
    input_bag.open(bag_path, rosbag::bagmode::Read); //打开bag并读取
    ...
    for (auto m : view) { //加载一遍bag中的图
        sensor_msgs::Image::ConstPtr pImage = m.instantiate<sensor_msgs::Image>(); //msgs::Image::ConstPtr pImage = m.instantiat
        if (pImage == nullptr) {
            cerr << endl
    ...
    vector<float> vTimesTrack;
    vTimesTrack.resize(nImages); //统计图片总数
    ...
    for (auto m : view) {
        double tframe = vTimestamps[ni];
        //usleep(879000);
        sensor_msgs::Image::ConstPtr pImage = m.instantiate<sensor_msgs::Image>(); //Compressed
        if (pImage == nullptr) {
            cerr << endl
                << "read finish or fail" << endl;
            break;
        ...
        try {
            cv_ptr = cv_bridge::toCvCopy(pImage);
        ...
        // Pass the image to the SLAM system
        std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
        TrackImage(cv_ptr->image, tframe, imageScale); //跳至449行,输入图像、时间戳、图像数
        std::chrono::steady_clock::time_point t2 = std::chrono::steady_clock::now();
