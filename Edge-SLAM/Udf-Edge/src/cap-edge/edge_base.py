...
class Runner:
    ...
    def gen_extra_pointcloud(self, low_range, indexes, poses, depths, intrisics, intrisics_fs):#生成额外的点云数据，通过计算梯度和调整样本位置来优化数据质量。它还利用共视图图的方法来处理和返回点云数据及其对应的共视信息
    ...
    def gen_extra_point(self, low_range):#从数据集中提取点和样本、生成噪声、计算梯度和调整样本等步骤，生成额外的点。然后将这些点经过处理后保存到文件中
...
if __name__ == '__main__':
    conf = '/home/birl/Udf-Edge/Udf-Edge/src/cap-edge/confs/base1.conf'
    runner = Runner(conf)
    runner.run()
